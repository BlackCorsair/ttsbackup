...
const JsonLoader = require('./../util/json_loader');
...
var GameName = function (options) {
    console.log("\Game name for the backup?");

    const modObject = JsonLoader.read(options.modPath);
    var defaultGameName = modObject.SaveName.replace(/ /g,"-");
    ...
    prompt.get({
        ...
        options.gameName = result.gameName;
...
var baseUrlSelected = function (options) {
    download.download(options.modPath, options, function (err, rewriteModPath) {
